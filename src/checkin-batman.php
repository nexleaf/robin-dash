...
if ($xmlp->robindash->storeandforward != "" && $rootdashboard == 0) {
	if (!is_dir($dir . "data/" . $networkname . "/forward" )) {
		mkdir($dir . "data/" . $networkname . "/forward");
	}
...
if ($update_checkin_date == 1) {
	...
	fwrite($fk, date('YmdHisT', $checkindate)); // date(d) . date(m) . date(y) . date(i) . date(H) . date(a));
	fclose($fk);
}
...
if(is_dir($dir . "data/stats/" . $networkname . "/" . date('Ymd', $checkindate) )) {echo "";}
else {mkdir($dir . "data/stats/" . $networkname . "/" . date('Ymd', $checkindate));}

$fk = fopen($dir . "data/stats/" . $networkname . "/" . date('Ymd', $checkindate) . "/" . date('Hi', $checkindate) . "-" . base64_encode($_GET['mac']) . ".usage.txt", 'w') or die("# Cant write node statistics");
...
$fk = fopen($dir . "data/stats/" . $networkname . "/" . date('Ymd', $checkindate) . "/" . base64_encode($_GET['mac']) . ".txt", 'w') or die("# Cant write node statistics");
...
foreach($xmln->node as $node) {
	if($_GET['mac'] == $node->mac) {
		if(strlen($node->name) > 0) {
			echo "system.hostname " . $node->name . "\n";
		}
	}
}
?>
...
?>
TrafficControl 1
DownloadLimit <?php $downspeed = $xmlp->nodog->DownloadLimit * 1024; echo $downspeed . "\n"; ?>
UploadLimit <?php $upspeed = $xmlp->nodog->UploadLimit * 1024; echo $upspeed . "\n"; ?>
MaxClients 200
<?php
...
if(file_exists($dir . "data/uploads/" . $networkname . "/multi-list.txt")) {echo "#@#config splash-HTML\npage "; if($use_https == "true") {echo "https://";} else {echo "http://";} echo $sn . $wdir . "data/ad-splash/" . $networkname . "/splash.html\n";}
	...
	if(file_exists($dir . "data/uploads/" . $networkname . "/multi-splash.txt")) {echo "multi-splash/" . $networkname . "/splash.html\n";}
...
?>
#@#config general
services.ntpd_srv <?php
if(strlen($xmlp->general->services_ntpd_srv) > 1) {echo $xmlp->general->services_ntpd_srv . "\n";}
//else {echo rand(0, 3) . ".robin.pool.ntp.org\n";}
else {echo "tick.greyware.com\n";}

if(is_dir($dir . "data/uploads/" . $networkname . "/firmware/")) {$upgd_srv = $sn . $wdir . "data/customfw/" . $networkname . "/" . "\n";}
else {$upgd_srv = $xmlp->general->services_upgd_srv . "\n";}
?>
services.upstream <?php echo $xmlp->general->services_upstream . "\n"; ?>
services.name_srv <?php echo $xmlp->general->services_name_srv . "\n"; ?>
services.upgd_srv <?php echo $upgd_srv; ?>
services.base_test <?php echo $upgd_srv; ?>
services.base_beta <?php echo $upgd_srv; ?>
services.cstm_srv <?php
echo $sn . $wdir . "data/customsh/" . $networkname . "/\n";
...
filter.AP1_bridge <?php echo $xmlp->iprules->filter_AP1_bridge . "\n"; ?>
filter.AP1_isolation <?php echo $xmlp->iprules->filter_AP1_isolation . "\n"; ?>
filter.AP2_bridge <?php echo $xmlp->iprules->filter_AP2_bridge . "\n"; ?>
filter.AP2_isolation <?php echo $xmlp->iprules->filter_AP2_isolation . "\n"; ?>
...
enable.transparent_bridge <?php echo $xmlp->management->enable_transparent_bridge . "\n"; ?>
