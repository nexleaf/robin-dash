...
$datatwo = "<node>\n<lat>" . $node->lat . "</lat>\n<lng>" . $node->lng . "</lng>\n<name>" . $node->name . "</name>\n<notes>" . $node->notes . "</notes>\n<ip>" . $node->ip . "</ip>\n<mac>" . $node->mac . "</mac>\n</node>";
				...
				if(file_exists($dir . "data/pnc/" . base64_encode($node->mac) . "/node.runmode.txt")) {unlink($dir . "data/pnc/" . base64_encode($node->mac) . "/node.runmode.txt");}
...
if($connectivity == "internet") {
	if(strlen($xmlp->robindash->location) > 0) {
...
if(isset($_GET['action'])) {echo "<script type=\"text/javascript\" src=\"http://maps.google.com/maps/api/js?sensor=false\"></script>";}
?>
...
function initialize() {
    ...
    var c = {
        zoom: <?php if(isset($_GET['action']) && $_GET['action'] == "configure") {echo "20";} else {echo "16";} ?>,
        center: b,
    ...
    document.getElementById("lat").value = 45;
    document.getElementById("lon").value = 15
}
